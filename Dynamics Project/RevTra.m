...
global Flag Jnt_RevTra
    ...
    if (Flag.Gamma==1)
        ...
        Gamma(Nline,1) = -dot(dd,body(j).B*Jnt_RevTra(k).hj*body(j).thetad)+dot(d,body(j).A*Jnt_RevTra(k).hj*body(j).thetad^2)...
    -dot(Jnt_RevTra(k).hj,body(j).A*Jnt_RevTra(k).spPj*body(j).thetad^2-body(i).A*Jnt_RevTra(k).spPi*body(i).thetad^2)...
    -dot(body(j).B*Jnt_RevTra(k).hj*body(j).thetad,-body(j).rd-body(j).B*Jnt_RevTra(k).spPj*body(j).thetad+body(i).rd+body(i).B*Jnt_RevTra(k).spPi*body(i).thetad); 
%  -dot(dd,body(j).B*Jnt_RevTra(k).spPj*body(j).thetad)+...
