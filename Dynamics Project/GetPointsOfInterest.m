...
for i=1:Nbody
    ...
    i2=i*3; % 3 6 9
    i1=i2-2; % 1 4 7
    bodytab{i}.Variables=[PositionsT(:,i1:i2),VelocitiesT(:,i1:i2),AccelerationsT(:,i1:i2)];
    bodytab{i}.Properties.VariableNames={'X','Y','THETA','XD','YD','THETAD','XDD','YDD','THETADD'};
end
...
for kt=1:length(t)
    ...
    for k = 1:Npointsint
        ...
        point= A(b.THETA(kt))*spP;
        p=center+point;
        i1=2*k-1;
        PosPOI(kt,i1:i1+1)=p';
        
        v_center=[b.XD(kt);b.YD(kt)];
        v=v_center+b.THETAD(kt)*[0 -1;1 0]*point;
        VelPOI(kt,i1:i1+1)=v';
        
        a_center=[b.XDD(kt);b.YDD(kt)];
        a=a_center-b.THETAD(kt)^2*point+ b.THETADD(kt)*[0 -1;1 0]*point;
        AccPOI(kt,i1:i1+1)=a;
    end
end
% close all
